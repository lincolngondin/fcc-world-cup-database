...
echo "$($PSQL "SELECT * FROM ((SELECT name FROM games LEFT JOIN teams ON teams.team_id=winner_id WHERE year = 2014 AND round='Eighth-Final') UNION (SELECT name FROM games LEFT JOIN teams ON teams.team_id=opponent_id WHERE year = 2014 AND round='Eighth-Final')) as aux ORDER by aux.name")"
